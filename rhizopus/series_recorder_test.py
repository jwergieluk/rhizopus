...
def some_t_x(t0, n: int = 50):
    ...
    t = set(t0 + datetime.timedelta(seconds=int(random.normalvariate(0.0, 2.0 * n))) for _ in range(n))
...
def test_series_recorder1():
    ...
    assert rec.get_recent_observations() == {'s1': s1[max(s1.keys())], 's2': s2[max(s2.keys())],
                                             's3': s3[max(s3.keys())]}
